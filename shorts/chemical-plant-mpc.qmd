...
Call of the solver is mostly trivial. If this is the first call to the solver, normally provide an initial guess composed of a fixed value that makes sense to your system (here an array of ones was chosen). When using the solver in an actual control loop, generally the results of last call are provided, what is interesting for speed-up.  Call can be a bit verbose and you might want to log it in a production environment.
...
In this brief section we propose a simple object-oriented wrapper encapsulating what has been developed so far. It is still a rough implementation, that would require many checks for adding robustness. Furthermore, it would be interesting to modify the code to simulate the problem only when required, as we have seen that the multiple-shooting approach considerably slows down the solution.
