...
confidence = 0.05
e *= st.norm.ppf(1.0 - confidence / 2.0)

stability = (e[1:] < tol_sem) & (v[1:] < tol_var)
stop_at = np.argmax(stability)
```

A summary of results is illustrated below. Shaded regions cover the initial window where results are not available for those quantites. The vertical blue line indicates de acquisition stop based on the above criteria.
